...
// Реализуйте и экспортируйте по умолчанию функцию select, 
// которая принимает на вход имя тега и html список, 
// а возвращает список всех нод, соответствующих имени. 
...
//     Если текущая нода содержит детей, то запускаем select рекурсивно для детей, 
...
import { /* eslint-disable */
  ...
  const select = (tagName, htmlList) => {
    const checkElem = (htmlL, acc) => {
      if (isEmpty(htmlL)) {
        return acc;
      }
      
      const htmlHead = head(htmlL);
      const htmlTail = tail(htmlL);
      
      if (!isList(htmlHead)) {
        const newAcc = is(tagName, htmlHead) ? consList(htmlHead, acc) : acc;
        return checkElem(htmlTail, newAcc);
        
      } else {
        return checkElem(
          htmlTail,
          checkElem(children(htmlHead),
          is(tagName, htmlHead) ? consList(htmlHead, acc) : acc));
      }
    };
    return checkElem(htmlList, l());
  };
  export default select;
  // END

  export const select = (tag, elements) => reduce((element, acc) => {
    const acc2 = hasChildren(element) ? concat(select(tag, children(element)), acc) : acc;
