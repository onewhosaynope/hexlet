// Реализуйте абстракцию для создания html. 
// Она включает в себя следующие функции:

//     make — конструктор. Уже реализован. 
//     Не принимает параметров, и создает html-список.
//     node — создает новый тег. 
//     Содержит два элемента, имя тега и его содержимое. 
//     Дополнительно реализуйте селекторы тега: 
//     getName и getValue.
...
//     append — добавляет элемент (тег), 
//     созданный с помощью node, в html-список. 
//     Возвращает новый html-список. 
//     Новый элемент должен добавляться в начало ("голову") списка.
//     toString — возвращает текстовое 
//     представление html на основании html-списка.
...
export const node = (tag_name, tag_val) => {
  return cons(tag_name, tag_val);
}
...
export const getValue = (tag) => {
  return cdr(tag)
} 

export const append = (html_l, tag) => {
  return consList(tag, html_l);
}
